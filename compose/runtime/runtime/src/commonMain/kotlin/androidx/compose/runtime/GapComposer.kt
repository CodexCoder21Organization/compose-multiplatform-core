...
internal class GapComposer(
     ...
     * composition position, nearest first: up the insertion writer's chain, then up the reader's
     * chain.
     */
    ...
    private inline fun walkCommittedErrorBoundaryMarkers(visit: (ErrorBoundaryMarker) -> Unit) {
        if (!reader.closed && reader.size != 0) {
            ...
            while (group >= 0) {
                if (
                    reader.groupKey(group) == errorBoundaryContentKey && reader.hasObjectKey(group)
                ) {
