...
import androidx.compose.ui.graphics.shadow.SkikoBlurFilterImplementation
...
/**
 * Skiko-side implementation entry point. Registration is intentionally kept separate.
 */
@OptIn(InternalComposeUiApi::class)
internal object SkikoGraphicsImpl : PlatformGraphics {
    ...
    override fun setBlurFilter(paint: Paint, blur: PlatformBlurFilter?) {
        require(blur == null || blur is SkikoBlurFilterImplementation) {
            "Expected SkikoBlurFilterImplementation, got ${blur?.let { it::class }}"
        }
    ...
    override fun createTintColorFilter(color: Color, blendMode: BlendMode): PlatformColorFilter =
        PlatformColorFilterImpl(SkColorFilter.makeBlend(color.toArgb(), blendMode.toSkia()))
    ...
    override fun createColorMatrixColorFilter(colorMatrix: ColorMatrix): PlatformColorFilter {
        ...
        return PlatformColorFilterImpl(SkColorFilter.makeMatrix(SkColorMatrix(remappedValues)))
    }

    override fun createLightingColorFilter(multiply: Color, add: Color): PlatformColorFilter =
        PlatformColorFilterImpl(SkColorFilter.makeLighting(multiply.toArgb(), add.toArgb()))
